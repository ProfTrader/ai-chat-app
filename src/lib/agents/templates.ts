...
export const libraryNodes: LibraryNode[] = [
  {
    ...
    title: "Ask questions",
    category: "Input",
    description: "Collects clarifying questions before a brief plan can be approved.",
    icon: MessageSquare,
    inputs: ["text"],
    outputs: ["json"],
    requiredConfig: ["Question set", "Completion rule", "Fallback assumption"],
    outputSchema: "{ questions: string[], answers: object, ready_to_plan: boolean }",
    risk: "low",
  },
  {
    id: "brief-retrieve",
    title: "Retrieve data",
    category: "Data",
    description: "Retrieves workspace tasks, contacts, sessions, and approved local sources.",
    icon: Database,
    inputs: ["json"],
    outputs: ["json", "memory"],
    requiredConfig: ["Workspace scope", "Source priority", "Missing-data policy"],
    outputSchema: "{ sources: object[], missing: string[], confidence: number }",
    risk: "medium",
  },
  {
    id: "brief-inspect",
    ...
    category: "Data",
    description: "Separates source evidence, assumptions, missing data, and confidence before drafting.",
    icon: ScanSearch,
    inputs: ["json", "memory"],
    outputs: ["json", "policy"],
    requiredConfig: ["Evidence schema", "Confidence threshold", "Unsupported-claim rule"],
    outputSchema: "{ evidence: object[], assumptions: string[], gaps: string[] }",
    risk: "low",
  },
  {
    id: "brief-analyze",
    ...
    category: "Agent",
    description: "Turns inspected evidence into findings, implications, and recommendations.",
    icon: Sigma,
    inputs: ["json", "memory"],
    outputs: ["json", "text"],
    requiredConfig: ["Analysis frame", "Audience", "Recommendation rule"],
    outputSchema: "{ findings: string[], recommendations: string[], confidence: number }",
    risk: "medium",
  },
  {
    id: "brief-visualize",
    ...
    category: "Data",
    description: "Creates KPI cards, tables, and charts from inspected evidence.",
    icon: BarChart3,
    inputs: ["json"],
    outputs: ["json"],
    requiredConfig: ["Chart types", "Data contract", "Insight labels"],
    outputSchema: "{ blocks: Array<{ type: string, title: string, data: object[] }> }",
    risk: "low",
  },
  {
    id: "brief-create",
    title: "Create brief",
    category: "Output",
    description: "Creates the HTML-style draft artifact with narrative, charts, tables, and source notes.",
    icon: FileText,
    inputs: ["text", "json"],
    outputs: ["text", "json"],
    requiredConfig: ["Artifact format", "Draft sections", "Versioning rule"],
    outputSchema: "{ draft_id: string, version: number, html_blocks: object[] }",
    risk: "medium",
  },
  {
    id: "brief-audit",
    title: "Audit brief",
    category: "Audit",
    description: "Scores plan completeness, source coverage, claim support, data validity, and readiness.",
    icon: ClipboardCheck,
    inputs: ["text", "json", "policy"],
    outputs: ["json", "policy"],
    requiredConfig: ["Checklist", "Passing score", "Loop-back rule"],
  ...
  },
  {
    ...
    category: "Output",
    description: "Prepares the final brief for chat, dashboard, HTML, PDF, or document delivery.",
    icon: Send,
    inputs: ["text", "json", "policy"],
    outputs: ["text"],
    requiredConfig: ["Destination", "Export format", "Approval gate"],
    outputSchema: "{ destination: string, artifact_url: string, status: enum }",
    risk: "medium",
  },
  {
    id: "dataset-intake",
    ...
    category: "Data",
    description: "Loads mock, CSV, or workspace datasets into the agent runtime with project scope.",
    icon: Database,
    inputs: ["json"],
    outputs: ["json", "memory"],
    requiredConfig: ["Data source", "Project scope", "Domain pack"],
    outputSchema: "{ dataset_id: string, source_kind: enum, rows: number, columns: number }",
    risk: "low",
  },
  {
    id: "schema-inspector",
    ...
    category: "Data",
    description: "Detects column types, missing values, date fields, entity fields, and semantic roles.",
    icon: ScanSearch,
    inputs: ["json"],
    outputs: ["json", "policy"],
    requiredConfig: ["Dataset", "Required roles", "Missing-data rule"],
    outputSchema: "{ columns: object[], missing: string[], required_roles_present: boolean }",
    risk: "low",
  },
  {
    id: "metric-analyst",
    ...
    category: "Agent",
    description: "Profiles numeric measures, KPI totals, averages, and distributions from mapped data.",
    icon: Calculator,
    inputs: ["json", "memory"],
    outputs: ["json", "text"],
    requiredConfig: ["Metric definitions", "Formula rules", "Unit policy"],
    outputSchema: "{ metrics: object[], anomalies: string[], confidence: number }",
    risk: "medium",
  },
  {
    id: "segment-analyst",
    title: "Segment analyst",
    category: "Agent",
    description: "Compares account stages, products, channels, authors, or owners against selected metrics.",
    icon: Layers3,
    inputs: ["json", "memory"],
    outputs: ["json", "text"],
    requiredConfig: ["Segment column", "Measure column", "Ranking rule"],
    outputSchema: "{ segments: object[], top_segment: string, risk_flags: string[] }",
    risk: "medium",
  },
  {
    id: "trend-analyst",
    title: "Trend analyst",
    category: "Agent",
    description: "Builds daily, weekly, or monthly trend reads from date and measure columns.",
    icon: BarChart3,
    inputs: ["json", "memory"],
    outputs: ["json", "text"],
    requiredConfig: ["Date column", "Measure column", "Window"],
    outputSchema: "{ series: object[], movement: string, cadence: string }",
    risk: "medium",
  },
  {
    id: "effectiveness-analyst",
    title: "Effectiveness analyst",
    category: "Agent",
    description: "Estimates directional lift for discounts, payouts, campaigns, or exposed cohorts.",
    icon: Scale,
    inputs: ["json", "memory"],
    outputs: ["json", "text"],
    requiredConfig: ["Exposure column", "Outcome metric", "Causality warning"],
  ...
  },
  {
    ...
    category: "Output",
    description: "Writes the sourced research memo, ops report, or dossier from tool-backed insights.",
    icon: FileText,
    inputs: ["json", "text"],
    outputs: ["json", "text"],
    requiredConfig: ["Artifact style", "Citation policy", "Audience"],
    outputSchema: "{ draft_id: string, findings: string[], citations: object[] }",
    risk: "medium",
  },
  {
    id: "audit-gate",
    title: "Audit gate",
    category: "Audit",
    description: "Blocks publishing until source coverage, metric validity, claim support, and actionability pass.",
    icon: ClipboardCheck,
    inputs: ["json", "policy"],
    outputs: ["json", "policy"],
    requiredConfig: ["Checklist", "Passing score", "Loop-back path"],
    outputSchema: "{ passed: boolean, score: number, required_fixes: string[] }",
    risk: "low",
  },
  {
    id: "task-proposal",
    title: "Task proposal",
    category: "Tool",
    description: "Turns approved findings into task, owner, board status, priority, and due-date proposals.",
    icon: ClipboardList,
    inputs: ["json", "policy"],
    outputs: ["json", "tool"],
    requiredConfig: ["Task schema", "Assignee matching", "Human review"],
    outputSchema: "{ proposed_tasks: object[], unresolved_owners: string[] }",
    risk: "medium",
  },
  {
    id: "action-trigger",
...
export const workflowTemplates = [
  {
    ...
    description: "Import data, inspect schema, analyze metrics, compare segments, trend movement, audit, and propose work.",
  ...
  {
    label: "Research brief loop",
    description: "Ask questions, retrieve workspace evidence, inspect gaps, create a brief, audit, and export.",
    domainPackId: "basic-workforce",
    icon: FileText,
    nodeIds: [
      "action-trigger",
